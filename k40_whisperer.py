...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        SaveEGVmenu.add(
            "command", label="Raster, Vector Engrave and Vector Cut", command=self.menu_File_Raster_Vector_Cut
        )
    ...
    def refreshTime(self):
        ...
        if self.GcodeData.ecoords == []:
            ...
            if Reng_time == None:
                ...
                self.calc_button.place_forget()
                self.PreviewCanvas.create_text(
                    HUD_X, HUD_Y - HUD_vspace * 2, fill="black", text=self.Reng_time.get(), anchor="se", tags="HUD"
                )
        else:
            self.PreviewCanvas.create_text(
    ...
    def bezier_plot(self):
        ...
        for i in range(0, num):
            self.BezierCanvas.create_line(
                5 + x[i], 260 - y[i], 5 + x[i + 1], 260 - y[i + 1], fill="black", capstyle="round", width=2, tags="bez"
            )
    ...
    def menu_File_save_EGV(self, operation_type=None, default_name="out.EGV"):
        ...
        filename = asksaveasfilename(
            defaultextension=".EGV", filetypes=[("EGV File", "*.EGV")], initialdir=init_dir, initialfile=init_file
        )
    ...
    def menu_File_Open_EGV(self):
        ...
        if not os.path.isdir(init_dir):
            init_dir = self.HOME_DIR
        fileselect = askopenfilename(
            filetypes=[("Engraver Files", ("*.egv", "*.EGV")), ("All Files", "*")], initialdir=init_dir
        )
        if fileselect != "" and fileselect != ():
            self.resetPath()
            self.DESIGN_FILE = fileselect
    ...
    def Open_SVG(self, filemname):
        ...
        if (
            self.Design_bounds[0] > self.VengData.bounds[0]
            or self.Design_bounds[0] > self.VcutData.bounds[0]
            or self.Design_bounds[1] < self.VengData.bounds[1]
            or self.Design_bounds[1] < self.VcutData.bounds[1]
            or self.Design_bounds[2] > self.VengData.bounds[2]
            or self.Design_bounds[2] > self.VcutData.bounds[2]
            or self.Design_bounds[3] < self.VengData.bounds[3]
            or self.Design_bounds[3] < self.VcutData.bounds[3]
        ):
            line1 = "Warning:\n"
    ...
    def menu_File_Save(self):
        ...
        filename = asksaveasfilename(
            defaultextension=".txt", filetypes=[("Text File", "*.txt")], initialdir=init_dir, initialfile=init_file
        )
            ...
            if (operation_type.find("Vector_Eng") > -1) and (self.VengData.ecoords != []):
                ...
                if not self.VengData.sorted and self.inside_first.get():
                    self.VengData.set_ecoords(
                        self.optimize_paths(self.VengData.ecoords, inside_check=False), data_sorted=True
                    )
                self.statusMessage.set("Generating EGV data...")
                self.master.update()
    ...
    def Plot_Data(self):
        ...
        if self.RengData.image != None:
            if self.include_Reng.get():
                ...
                self.Plot_Raster(
                    self.laserX + 0.001, self.laserY - 0.001, x_lft, y_top, self.PlotScale, im=self.UI_image
                )
        else:
            self.UI_image = None
    ...
    def Plot_Raster(self, XX, YY, Xleft, Ytop, PlotScale, im):
        ...
        self.segID.append(self.PreviewCanvas.create_image(xplt, yplt, anchor=NW, image=self.UI_image, tags="LaserTag"))
        ...
        if cross_hair:
            ...
            self.segID.append(
                self.PreviewCanvas.create_line(
                    xplt - radius, yplt, xplt + radius, yplt, fill=col, capstyle="round", width=1, tags=circle_tags
                )
            )
            self.segID.append(
                self.PreviewCanvas.create_line(
                    xplt, yplt - radius, xplt, yplt + radius, fill=col, capstyle="round", width=1, tags=circle_tags
                )
            )
        else:
            self.segID.append(
    ...
    def Plot_Line(
        self, XX1, YY1, XX2, YY2, Xleft, Ytop, XlineShift, YlineShift, PlotScale, col, thick=0, tag_value="LaserTag"
    ):
    ...
    def RASTER_Settings_Window(self):
        ...
        self.Halftone_DPI_OptionMenu = OptionMenu(
            raster_settings, self.ht_size, "1000", "500", "333", "250", "200", "167", "143", "125"
        )
        ...
        self.bezier_M1_Slider = Scale(
            raster_settings, from_=1, to=50, resolution=0.1, orient=HORIZONTAL, variable=self.bezier_M1
        )
        ...
        self.bezier_M2_Slider = Scale(
            raster_settings, from_=0.0, to=1, orient=HORIZONTAL, resolution=0.01, variable=self.bezier_M2
        )
        ...
        self.bezier_weight_Slider = Scale(
            raster_settings, from_=0, to=10, resolution=0.1, orient=HORIZONTAL, variable=self.bezier_weight
        )
...
class pxpiDialog(tkSimpleDialog.Dialog):
    def __init__(self, parent, units="mm", SVG_Size=None, SVG_ViewBox=None, SVG_inkscape_version=None):
