...
class NURBSClass:
    def __init__(self, degree=0, Knots=[], Weights=None, CPoints=None):
        ...
        self.BSpline = BSplineClass(degree=self.degree, Knots=self.Knots, CPts=self.HCPts)

    # Calculate a number points using error limiting
    def calc_curve(self, n=0, lin_tol=0.001):
...
class DXF_CLASS:
    ...
    def bulge_coords(self, x0, y0, x1, y1, bulge, lin_tol=0.001):
        global Zero
        bcoords = []
        if bulge < 0.0:
            sign = 1
    ...
    def eval_entity(self, e, bl, lin_tol=0.001, offset=[0, 0], scale=[1, 1], rotate=0):
        try:
            color = e.data["62"]
        except:
            color = None
        ...
        ############ SPLINE ###########
            ...
            if xy_data != []:
                ...
                self.MYNURBS = NURBSClass(
                    degree=self.degree, Knots=self.Knots, Weights=self.Weights, CPoints=self.CPoints
                )
    ...
    def GET_DXF_DATA(self, fd, lin_tol=0.001, get_units=False, units=None):
        data = []
        try:
